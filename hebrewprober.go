...
// HPNormalChars are Non final code points
...
var HPNormalChars = []byte{HPNormalKaf, HPNormalMem, HPNormalNun, HPNormalNun, HPNormalPe}

// HPFinalChars Final code points
var HPFinalChars = []byte{HPFinalKaf, HPFinalMem, HPFinalNun, HPFinalPe, HPFinalTsadi}

// HPMinFinalCharDistance Minimum Visual vs Logical final letter score difference.
// If the difference is below this, don't rely solely on the final letter score
// distance.
var HPMinFinalCharDistance int = 5

// HPMinModelDistance Minimum Visual vs Logical model score difference.
...
func (h *HebrewProber) reset() {
	...
	// TODO WHY?? Why not (re-)initialize the probers??
}

func (h *HebrewProber) setModelProbers(logicalProber, visualProber *SingleByteCharSetProber) {
	h.logicalProber = logicalProber
	h.visualProber = visualProber
}

func (h *HebrewProber) isFinal(c byte) bool {
	for _, f := range HPFinalChars {
		if c == f {
			return true
		}
	}
...
func (h *HebrewProber) isNonFinal(c byte) bool {
	for _, f := range HPNormalChars {
		if c == f {
			return true
		}
	}
...
func (h *HebrewProber) state() ProbingState {
	if h.logicalProber.state == PSNotMe && h.visualProber.state == PSNotMe {
		return PSNotMe
	}
	return PSDetecting
}

func (h *HebrewProber) charsetName() string {
	// Make the decision: is it Logical or Visual?
	// If the final letter score distance is dominant enough, rely on it.
	finalSub := h.finalCharLogicalScore - h.finalCharVisualScore
	if finalSub >= HPMinFinalCharDistance {
		return HPLogicalHebrewName
	}
	if finalSub <= -HPMinFinalCharDistance {
		return HPVisualHebrewName
	}

	// It's not dominant enough, try to rely on the model scores instead.
	modelSub := h.logicalProber.getConfidence() - h.visualProber.getConfidence()
	if modelSub > HPMinModelDistance {
		return HPLogicalHebrewName
	}
	if modelSub < -HPMinModelDistance {
		return HPVisualHebrewName
...
func (h *HebrewProber) feed(data []byte) ProbingState {
	...
	newData := filterHighByteOnly(data)
	...
	for _, chr := range newData {
		if chr == ' ' {
			...
			if h.beforePrev == ' ' && h.isFinal(h.prev) && chr != ' ' {
...
// WARNING THIS METHOD WAS ADDED WHEN MIGRATING THE PROJECT TO GO
// It makes sense as it takes the logic from the language detection,
// but it should be subject to caution
func (h *HebrewProber) getConfidence() float64 {
	logicalConf := h.logicalProber.getConfidence()
	visualConf := h.visualProber.getConfidence()
	...
	// It's not dominant enough, try to rely on the model scores instead.
	modelSub := logicalConf - visualConf
	if modelSub > HPMinModelDistance {
		return logicalConf
	}
	if modelSub < -HPMinModelDistance {
